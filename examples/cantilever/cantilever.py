...
mesh = fd.Mesh(f"{dir_path}/mesh_cantilever.msh")
...
def compliance_optimization(mesh, phi, n_iters=200):
    ...
    return results, phi, hs(phi, beta)

from animate import RiemannianMetric, adapt

fpvd = fd.File('phi.pvd', adaptive=True)
for i in range(20):

    results, phi, hs = compliance_optimization(mesh, phi, 5)
    hsfunc = fd.Function(PHI, name='material')
    hsfunc.interpolate(hs)
    fpvd.write(phi, hsfunc)

    TV = fd.TensorFunctionSpace(mesh, "CG", 1)
    metric = RiemannianMetric(TV)
    metric.compute_hessian(hs)
    metric.set_parameters({
        'dm_plex_metric_h_min': 0.01,
        'dm_plex_metric_h_max': 0.1,
    ...
    PHI = fd.FunctionSpace(mesh, "CG", 1)
    phi_new = fd.Function(PHI, name="LevelSet")
    phi_new.interpolate(phi)
    phi = phi_new
